...
| **Live deployment** | Done | Running on the VPS behind CyberPanel/OpenLiteSpeed at `mykiz.my` (single VPS, PostgreSQL 16, `mykiznext` systemd service on `127.0.0.1:3010`). Deploy = `./deploy.sh` (lint → auto-commit → push to `main`) → GitHub Action → `scripts/remote-deploy.sh` on the server (pull, npm ci, prisma db push, build, restart). `.env` + uploads stay server-side only — the GitHub repo is public. **The #1 VPS-only gotcha (root-caused via `journalctl`):** if `AUTH_URL` is missing/malformed in the server `.env`, every `auth()` call inside a Server Action crashes with `TypeError: Invalid URL` → `500 Internal Server Error` on **all** saves/uploads. Auth.js v5 (`@auth/core/lib/utils/env.js` → `createActionURL`) falls back to `new URL(proto + "//" + x-forwarded-host)` when `AUTH_URL` is unset, and OpenLiteSpeed forwards `x-forwarded-host` as a malformed comma-joined value (`mykiz.my, https://mykiz.my`) → `new URL("https://mykiz.my, https://mykiz.my")` throws. `remote-deploy.sh` now idempotently ensures `AUTH_URL=https://mykiz.my` on every deploy. Secondary hardening: `experimental.serverActions.allowedOrigins: ["mykiz.my", "www.mykiz.my"]` (Next 16 honours this — `action-handler.js` `isCsrfOriginAllowed` skips the host-mismatch check) and `proxy.ts` normalising `x-forwarded-host`. |
