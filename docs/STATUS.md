...
| **Live deployment** | Done | Running on the VPS behind CyberPanel/OpenLiteSpeed at `mykiz.my` (single VPS, PostgreSQL 16, `mykiznext` systemd service on `127.0.0.1:3010`). Deploy = `./deploy.sh` (lint → auto-commit → push to `main`) → GitHub Action → `scripts/remote-deploy.sh` on the server (pull, npm ci, prisma db push, build, restart). `.env` + uploads stay server-side only — the GitHub repo is public. Note: the server `.env` must set `AUTH_URL=https://mykiz.my` — Next/Auth.js otherwise builds redirect URLs from the backend origin (`localhost:3010`) instead of the public host. The proxy also doesn't forward the original `Host`, so Next's Server-Action CSRF check (Origin `mykiz.my` vs host `localhost:3010`) was aborting **every** Server Action. **Real fix:** `experimental.serverActions.allowedOrigins: ["mykiz.my", "www.mykiz.my"]` in `next.config.ts`. Next 16 *does* honour this — `action-handler.js` calls `isCsrfOriginAllowed(origin, allowedOrigins)` and when the browser Origin is listed it skips the host-mismatch check entirely (verified against the Next source). The earlier note that this field was "dead / never wired to the runtime" was **wrong** and caused this bug to linger; the `proxy.ts` `x-forwarded-host` injection is kept as belt-and-suspenders but `allowedOrigins` is the reliable fix that makes logo upload + all settings saves work on the VPS. |
