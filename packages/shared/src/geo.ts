/** Geo helpers for the AR Directory. Pure functions — safe everywhere. */
...
/** Turn instruction for a heading delta, e.g. "Turn right". */
export function turnHint(turnDeg: number): string {
  const abs = Math.abs(turnDeg)
  if (abs < 12) return "Straight ahead"
  if (abs < 45) return turnDeg > 0 ? "Slightly right" : "Slightly left"
  if (abs < 135) return turnDeg > 0 ? "Turn right" : "Turn left"
  return "Turn around"
}
