/**
 * Laundry meta — machine states, cycle presets and pure status derivation.
 *
 * There is no laundry-machine API, so a machine's "live" status is inferred from
 * the reminder residents set. This module is pure (no Prisma, no server imports)
 * so both the member client and the admin client can share it; the DB reads live
 * in `lib/laundry.ts`.
 */
import type { LaundryReminderEndReason } from "@/app/generated/prisma/client"
...
export interface LaundryReminderInput {
  ...
  endedReason?: LaundryReminderEndReason | null
}
